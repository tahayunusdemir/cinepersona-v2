...
export async function generateMetadata({
  ...
  return { title: `Following · @${username}`, robots: { index: false } };
}
...
export default async function FollowingPage({
  ...
  const ids = ((edges ?? []) as { following_id: string }[]).map(
    (r) => r.following_id,
  );
      ...
      {people.length === 0 ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              When this user follows other members, they will appear here.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
